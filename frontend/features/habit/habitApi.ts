export const fetchHabits = async(token : string) => {
    const response = await fetch('https://backend-pabloalfonsovargas-20034951s-projects.vercel.app/habits', {
        headers : {
...
export const markHabitAsDone = async(id: string, token: string) => {
    const response = await fetch(`https://backend-pabloalfonsovargas-20034951s-projects.vercel.app/habits/done/${id}`, {
        method: 'PATCH',
        headers : {
            Authorization : `Bearer ${token}`,

export const fetchHabits = async(token : string) => {
    const response = await fetch('https://backend-sigma-one-34.vercel.app/habits', {
        headers : {
...
export const markHabitAsDone = async(id: string, token: string) => {
    const response = await fetch(`https://backend-sigma-one-34.vercel.app/habits/done/${id}`, {
        method: 'PATCH',
        headers : {
            Authorization : `Bearer ${token}`,
